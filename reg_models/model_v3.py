...
data_dict = {
    ...
    'debt_to_equity_2023': [0.85, 0.25, 1.45, 0.12, 0.95, 0.68, 0.55, 0.40, 0.35, 0.26, 0.75, 0.60, 
                            0.45, 0.70, 0.55, 0.30, 0.50, 0.65, 0.40, 0.35, 0.45, 0.80, 0.35, 0.55, 
                            0.18, 0.50, 0.45, 0.20, 0.60, 0.40, 0.30, 0.75, 1.10, 0.45, 0.50, 0.55, 
                            0.70, 0.65, 0.40, 0.45, 0.85, 0.30, 0.90, 0.75, 0.55, 0.40, 0.35, 0.50, 
                            0.80, 1.05],
    'revenue_2023': [2544.7, 800.1, 712.3, 5483.0, 234.5, 612.4, 312.8, 1890.0, 456.2, 3520.0, 512.8, 
                     2450.0, 589.4, 480.1, 789.0, 189.5, 312.4, 501.2, 145.6, 312.0, 289.4, 178.9, 
                     312.5, 890.0, 2456.0, 456.2, 312.8, 45.6, 189.0, 145.2, 312.8, 456.1, 890.4, 
                     3124.0, 456.2, 189.5, 512.3, 456.1, 312.8, 189.4, 890.2, 1456.0, 512.4, 890.1, 
                     312.8, 456.2, 3520.0, 2544.7, 456.2, 890.4],
    'ebitda_2023': [166.1, 97.0, 312.4, 2340.0, 45.6, 189.5, 45.2, 312.8, 89.4, 912.3, 189.5, 456.2, 
                    89.4, 145.6, 312.4, 45.6, 89.4, 189.5, 45.6, 145.6, 45.2, 45.6, 89.4, 312.8, 
                    890.2, 89.4, 89.4, 12.3, 45.6, 89.4, 145.6, 89.4, 189.5, 890.1, 145.6, 45.6, 
                    189.5, 145.6, 89.4, 45.6, 312.4, 456.2, 189.5, 312.8, 89.4, 145.6, 912.3, 166.1, 
                    89.4, 189.5]
}
...
industry_map = {
    ...
    'OilGas': 'Other services',
    'Food': 'Other services',
    'Mining': 'Other services',
    'Construction': 'Other services',
    'Steel': 'Other services',
    'Auto': 'Other services',
    'Health': 'Other services',
    'Other': 'Other services'
}
data['industry'] = data['industry'].map(industry_map)
data['delta_ln_margin'] = np.log(data['margin_2024'].replace(0, np.nan)) - np.log(data['margin_2023'].replace(0, np.nan))
data['delta_ai_score_std'] = (data['delta_ai_score'] - data['delta_ai_score'].mean()) / data['delta_ai_score'].std()

data = pd.get_dummies(data, columns=['industry'], drop_first=True, dtype=int)

data['ln_age'] = np.log(data['age_2023'])
data['ln_revenue_2023'] = np.log(data['revenue_2023'].replace(0, np.nan))
data['ln_ebitda_2023'] = np.log(data['ebitda_2023'].replace(0, np.nan))
controls = [
    # 'investment_to_revenue_2023', 
    # 'ln_age', 
    # 'debt_to_equity_2023',
    # 'ln_revenue_2023',
    'ln_ebitda_2023'
] + [col for col in data.columns if col.startswith('industry_')]

def run_regression():
    clean_data = data.dropna(subset=['delta_ln_margin', 'delta_ai_score_std'] + controls)
    
    X = clean_data[['delta_ai_score_std'] + controls]
    X = sm.add_constant(X)
    y = clean_data['delta_ln_margin']
    
    model = sm.OLS(y, X).fit()
    print("Результаты регрессионного анализа (с упрощённой отраслью + новыми финансовыми переменными из отчётов 2023):")
    print(model.summary())
    return model
